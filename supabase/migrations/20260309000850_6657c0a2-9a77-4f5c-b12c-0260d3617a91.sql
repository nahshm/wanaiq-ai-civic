...
BEGIN
  ...
  WITH combined_feed AS (
    ...
    SELECT
      ...
      -- +1 ensures fresh posts with 0 votes still surface; score decays with age
      (GREATEST(p.upvotes - p.downvotes, 0) + 1) * EXP(-0.003 * EXTRACT(EPOCH FROM (NOW() - p.created_at)) / 3600) AS hot_score,
