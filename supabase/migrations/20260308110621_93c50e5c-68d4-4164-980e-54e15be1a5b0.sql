...
-- Drop all existing overloads of get_unified_feed
DROP FUNCTION IF EXISTS public.get_unified_feed(uuid, int, int);
DROP FUNCTION IF EXISTS public.get_unified_feed(uuid, int, int, text);
DROP FUNCTION IF EXISTS public.get_unified_feed(uuid, int, int, text, uuid);

-- Recreate with full signature
...
-- Grant access
GRANT EXECUTE ON FUNCTION public.get_unified_feed(uuid, int, int, text, uuid) TO anon;
GRANT EXECUTE ON FUNCTION public.get_unified_feed(uuid, int, int, text, uuid) TO authenticated;
GRANT EXECUTE ON FUNCTION public.get_unified_feed(uuid, int, int, text, uuid) TO service_role;

COMMENT ON FUNCTION public.get_unified_feed IS 'Unified feed with sorting (hot/new/top/rising) and optional community_id filtering. Aggregates posts, projects, and feed_activities.';
